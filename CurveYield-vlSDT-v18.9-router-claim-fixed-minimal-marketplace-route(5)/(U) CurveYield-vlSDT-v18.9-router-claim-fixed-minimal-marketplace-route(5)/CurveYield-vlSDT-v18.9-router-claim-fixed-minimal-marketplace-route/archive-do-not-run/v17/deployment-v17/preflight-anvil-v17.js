...
const { ethers, loadConfig, statePath } = require("./lib-v17");
const { deployAndConfigure } = require("./deploy-configure-v17");
const { verifyDeployment } = require("./verify-deployment-v17");
const { proposeHandoff } = require("./propose-handoff-v17");
...
async function runPreflight(options = {}) {
  ...
  try {
    ...
    await proposeHandoff({ configPath: options.configPath || process.argv[2], rpcUrl: url, privateKey: deployerKey, tag, simulation: true });
    ...
    for (const [key, entry] of Object.entries(ctx.state.contracts)) {
      const abi = require("./lib-v17").loadArtifact(entry.name).abi;
      const c = new ethers.Contract(entry.address, abi, finalSigner);
      if ((await c.owner()).toLowerCase() !== config.finalOwner.toLowerCase()) await (await c.acceptOwnership()).wait();
    }
