#!/usr/bin/env node
"use strict";
const { makeContext, contract, send, CONTRACTS, saveState } = require("./lib-v17");
const { verifyDeployment } = require("./verify-deployment-v17");

async function proposeHandoff(options = {}) {
  const ctx = await makeContext({ configPath: options.configPath || process.argv[2], rpcUrl: options.rpcUrl || process.env.RPC_URL, privateKey: options.privateKey || process.env.DEPLOYER_PRIVATE_KEY, tag: options.tag || process.env.DEPLOYMENT_TAG || "live" });
  ...
  console.log("ownership proposed; final owner must run accept-handoff-v17.js");
  return ctx;
}
