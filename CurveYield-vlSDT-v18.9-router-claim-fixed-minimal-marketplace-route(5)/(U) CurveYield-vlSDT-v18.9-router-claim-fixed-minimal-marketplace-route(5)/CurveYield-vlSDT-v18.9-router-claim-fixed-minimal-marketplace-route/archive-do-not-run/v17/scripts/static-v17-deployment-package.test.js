...
const gov = read("contracts/CurveYieldGovernanceStakingV17.sol");
const rev = read("contracts/CurveYieldVlSDTRevenueStakingV17.sol");
const locker = read("contracts/CurveYieldVlSDTLockerV17.sol");
const compounder = read("contracts/CurveYieldRevenueCompounderV17.sol");
const all = fs.readdirSync(path.join(root, "contracts")).filter(x => /^CurveYield.*V17\.sol$/.test(x)).map(x => read(`contracts/${x}`)).join("\n");
...
const config = JSON.parse(read("config-mainnet-v17.json"));
...
for (const script of ["preflight-anvil-v17.js", "deploy-configure-v17.js", "verify-deployment-v17.js", "propose-handoff-v17.js", "accept-handoff-v17.js"]) {
  must(fs.existsSync(path.join(root, "deployment-v17", script)), `${script} missing`);
}
const deploymentText = fs.readdirSync(path.join(root, "deployment-v17")).filter(x => x.endsWith(".js")).map(x => read(`deployment-v17/${x}`)).join("\n");
...
console.log("V17 deployment-package static requirements passed");
