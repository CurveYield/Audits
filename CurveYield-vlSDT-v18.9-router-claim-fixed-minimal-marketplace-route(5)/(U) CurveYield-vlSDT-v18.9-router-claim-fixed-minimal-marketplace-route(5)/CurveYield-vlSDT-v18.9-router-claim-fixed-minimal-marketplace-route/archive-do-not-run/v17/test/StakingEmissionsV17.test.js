...
describe("DAO-set governance emissions V17", function () {
  ...
  it("mints governance rewards on demand for Boost Staking", async function () {
    ...
    const Cyvl = await ethers.getContractFactory("MockCyvlSdtV17");
    const cyvl = await Cyvl.deploy();
    const Sdt = await ethers.getContractFactory("MockERC20V17");
    ...
    const Gov = await ethers.getContractFactory("CurveYieldGovernanceTokenV17");
    const gov = await Gov.deploy(dao.address, "CurveYield Governance", "cyGOV");
    const Boost = await ethers.getContractFactory("CurveYieldVlSDTBoostStakingV17");
