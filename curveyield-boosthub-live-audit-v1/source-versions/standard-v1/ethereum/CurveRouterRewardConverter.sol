...
interface ICurveRouterRewardConverterRouter {
...
contract CurveRouterRewardConverter is ReentrancyGuard {
    ...
    constructor(
        ...
        if (tokenIn_ == address(0) || tokenOut_ == address(0) || router_ == address(0)) revert InvalidToken();
        if (route__[0] != tokenIn_) revert InvalidRoute();
        ...
        returns (uint256 amountOut)
    {
        ...
        amountOut =
            ICurveRouterRewardConverterRouter(router).get_exchange_multiple_amount(route_, swapParams_, amountIn);
    }
        ...
        returns (uint256 amountOut)
    {
        ...
        ICurveRouterRewardConverterRouter(router).exchange_multiple(route_, swapParams_, amountIn, minAmountOut);
