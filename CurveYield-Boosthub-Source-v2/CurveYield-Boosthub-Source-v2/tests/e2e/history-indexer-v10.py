...
with sync_playwright() as p:
    ...
    page.wait_for_function("() => document.querySelector('.yield-history-panel')?.innerText.includes('2 observed points')", timeout=30000)
    panel=page.locator('.yield-history-panel').inner_text()
    svg=page.locator('.yield-history-panel svg.yield-chart')
    add('remote D1 history renders instead of the empty local fallback', svg.count() == 1 and '2 observed points' in panel, panel)
