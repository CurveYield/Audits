 ...
 * 3. Deploy and configure V20 through the canonical deployAndConfigure path.
...
function makeActor(provider, label) {
  const privateKey = ethers.keccak256(
    ethers.toUtf8Bytes(`CurveYield-v20-focused-vlsdt-claim:${label}`)
  );
...
async function runFocusedSimulation(options = {}) {
  ...
  const tag = `focused-vlsdt-claim-30d-v20-${Date.now()}`;
  const report = {
