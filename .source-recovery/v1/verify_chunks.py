...
request = json.loads((root / 'REQUEST-v1.json').read_text())
manifest = json.loads(Path(os.environ.get('CHUNK_MANIFEST_PATH', root / 'CHUNK-MANIFEST-v1.json')).read_text())
expected_request = {
    'schemaVersion': 'curveyield-source-recovery-request-v1',
    ...
    'chunkManifest': '.source-recovery/v1/CHUNK-MANIFEST-v1.json',
}
if request != expected_request:
    raise SystemExit('recovery request mismatch')
if manifest.get('schemaVersion') != 'curveyield-source-recovery-chunks-v1':
