...
must("contracts/CurveYieldRevenueVaultV20.sol", [
...
must("contracts/CurveYieldRevenueStrategyV20.sol", [
  "contract CurveYieldRevenueStrategyV7",
  "function beforeDeposit()",
  "function beforeDepositStrict()",
  "function beforeWithdraw()",
  "function harvestBeforeWithdraw()",
...
must("deployment-v20/deploy-configure-v20.js", [
...
must("deployment-v20/verify-deployment-v20.js", [
...
const migrationTests = read("test/v20/RevenueVaultV20.test.js");
for (const text of [
  ...
  "harvests ordinary rewards before withdrawal pricing",
  "continues withdrawal when the pre-withdraw harvest fails"
]) {
  if (!migrationTests.includes(text)) throw new Error(`migration regression test missing: ${text}`);
}

const vaultSource = read("contracts/CurveYieldRevenueVaultV20.sol");
const withdrawSource = vaultSource.slice(vaultSource.indexOf("function withdraw(uint256 shares)"), vaultSource.indexOf("function proposeStrat"));
if (withdrawSource.indexOf("strategy.beforeWithdraw();") === -1 || withdrawSource.indexOf("assets = Math.mulDiv(balance()") === -1
    || withdrawSource.indexOf("strategy.beforeWithdraw();") > withdrawSource.indexOf("assets = Math.mulDiv(balance()")) {
  throw new Error("pre-withdraw harvest must run before realized withdrawal pricing");
}

const config = JSON.parse(read("config-mainnet-v20.json"));
if (config.release !== "20") throw new Error("V20 config required");
...
) throw new Error("V20 revenue strategy defaults mismatch");
...
absent("contracts/CurveYieldRevenueStrategyV20.sol", ["GOVERNANCE_TOKEN.forceApprove(address(CONVERTER)"]);
...
console.log("V20 Beefy revenue-vault and immediate converter-route static checks passed.");
