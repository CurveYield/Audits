...
const tests = path.join(root, "test", "v20");
...
const revenueVaultFile = path.join(contracts, "CurveYieldRevenueVaultV20.sol");
const revenueStrategyFile = path.join(contracts, "CurveYieldRevenueStrategyV20.sol");
...
for (const required of [contracts, tests, archive, stakingFile, strategyFile, governanceMintControllerFile, revenueVaultFile, revenueStrategyFile, revenueConverterFile, cyGovDistributorFile, cyGovYieldStakingFile]) {
  if (!fs.existsSync(required)) fail(`missing required V20 path: ${required}`);
}
...
console.log("V20 active source layout and architecture checks passed.");
