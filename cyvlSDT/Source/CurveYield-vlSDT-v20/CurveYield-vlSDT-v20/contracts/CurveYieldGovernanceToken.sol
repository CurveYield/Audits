...
contract CurveYieldGovernanceToken is ERC20, ERC20Capped, Ownable2Step {
    ...
    /// @notice Releases a protected quota-controlled reservation whose recorded minter was removed.
    /// @dev This is deliberately narrower than the ordinary owner cancellation path: active minters
    ///      retain protection, and non-protected/non-quota reservations cannot use this recovery path.
    function cancelRemovedMinterReservation(uint256 id) external onlyOwner {
        MintReservation storage reservation = mintReservations[id];
        if (
            !reservation.open
                || !reservation.quotaControlled
