...
function loadConfig(configPath) {
  const resolved = path.resolve(configPath || path.join(ROOT, "config-mainnet-v20.json"));
  const c = readJson(resolved);
  if (c.version !== 20 || c.release !== "20" || c.chainId !== 1) {
    throw new Error("V20 Ethereum-mainnet config required");
  }
  ...
  for (const [key, expected] of Object.entries(expectedAllocations)) {
    ...
    if (initialCap !== expected.initialCap || additionalBps !== expected.additionalBps) {
      throw new Error(`governanceMinting.allocations.${key} does not match the approved V20 original quota`);
    }
...
function artifactPath(name) {
  return path.join(ROOT, "artifacts-v20", "contracts", `${name}.sol`, `${name}.json`);
}

function loadArtifact(name) {
  const artifactFile = artifactPath(name);
  if (!fs.existsSync(artifactFile)) {
    throw new Error(`Missing artifact ${artifactFile}. Run npm run compile:v20 first.`);
  }
...
function statePath(chainId, tag = "live") {
  return path.join(ROOT, "deployment-output-v20", `deployment-state-v20-${tag}-${chainId}.json`);
}
