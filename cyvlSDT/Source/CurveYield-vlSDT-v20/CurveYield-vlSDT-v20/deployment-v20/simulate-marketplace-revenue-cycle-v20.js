...
/**
 * Focused V20 marketplace revenue-cycle simulation.
 *
...
const { waitForRpc } = require("./simulate-all-functions-v20");
const {
  setEthBalance,
  withImpersonatedSigner,
  fundErc20FromRealHolder
} = require("./simulation-v20/actors-v20");
...
function makeActor(provider, label) {
  const privateKey = ethers.keccak256(
    ethers.toUtf8Bytes(`CurveYield-v20-marketplace-simulation:${label}`)
  );
...
function markdownReport(report) {
  ...
  const lines = [
    "# CurveYield V20 Marketplace Revenue Cycle",
    "",
    `**Status:** ${status}`,
    "",
...
async function runMarketplaceRevenueCycle(options = {}) {
  ...
  const reviewedScript = path.join(ROOT, "deployment-v20", "simulate-live-deployment-30d-v20.js");
  ...
  const tag = options.tag || `marketplace-revenue-cycle-v20-${Date.now()}`;
  const outputDir = path.join(ROOT, "deployment-output-v20");
