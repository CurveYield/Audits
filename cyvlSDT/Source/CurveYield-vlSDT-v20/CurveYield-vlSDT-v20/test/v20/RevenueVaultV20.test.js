...
describe("CurveYield Beefy V7 revenue vault", function () {
  ...
  it("harvests ordinary rewards before withdrawal pricing", async function () {
    const ctx = await fixture();
    await ctx.strategy.connect(ctx.dao).setFees(0, 0, 0);
    await addSdtReward(ctx, ethers.parseEther("100"));
    ...
    expect(await ctx.revenue.rewardEarned(await ctx.strategy.getAddress(), await ctx.sdt.getAddress())).to.equal(0);
    expect(received).to.equal(ethers.parseEther("199"));
    expect(await ctx.vault.totalSupply()).to.equal(0);
  });

  it("continues withdrawal when the pre-withdraw harvest fails", async function () {
    const ctx = await fixture();
    await ctx.strategy.connect(ctx.dao).setFees(0, 0, 0);
    await addSdtReward(ctx, ethers.parseEther("100"));
    ...
    expect(received).to.equal(ethers.parseEther("99.5"));
    expect(await ctx.revenue.rewardEarned(await ctx.strategy.getAddress(), await ctx.sdt.getAddress())).to.equal(ethers.parseEther("100"));
    expect(await ctx.vault.totalSupply()).to.equal(0);
  });
