...
/**
 * CurveYield vlSDT V18.9 — live-connected 30-day deployment simulation
 *
 * PURPOSE
 * -------
 ...
 *   2. deploys and configures the complete V18.9 system through the canonical
 *      deploy-configure-v18.js path;
 ...
 * ------------------------------------------------------
 * config-mainnet-v18.json currently contains zero governance emission/yield rates
 ...
 *   node deployment-v18/simulate-live-deployment-30d-v18.9.js config-mainnet-v18.json
 */
...
function diagnosticStage(label) {
  console.log(`[diagnostic-stage] ${label}`);
}

async function attachDiagnosticRpcTrace(provider) {
  await provider.on("debug", event => {
    if (event.action === "sendRpcPayload") {
      ...
      for (const payload of payloads) {
        console.log(`[diagnostic-rpc] send id=${payload.id} method=${payload.method}`);
      }
      return;
    }
    if (event.action === "receiveRpcResult") {
      ...
      for (const result of results) {
        console.log(`[diagnostic-rpc] receive id=${result.id}`);
      }
      return;
    }
    if (event.action === "receiveRpcError") {
      console.log(`[diagnostic-rpc] error ${errorText(event.error)}`);
    }
  });
}
...
function makeActor(provider, label) {
  const privateKey = ethers.keccak256(ethers.toUtf8Bytes(`CurveYield-v18.9-live-simulation:${label}`));
...
async function verifyLiveWiring(ctx) {
  ...
  assertAddress(await locker.FEE_DISTRIBUTOR(), c.stakeDao.feeDistributor, "locker.FEE_DISTRIBUTOR");
  ...
  const feeDistributor = new ethers.Contract(c.stakeDao.feeDistributor, FEE_DISTRIBUTOR_ABI, ctx.provider);
  const rewardTokenAddress = ethers.getAddress(await feeDistributor.REWARD_TOKEN());
...
async function runLiveDeploymentSimulation(options = {}) {
  ...
  let ctx;
  let reportPath;
  const tag = `live-30d-v18.9-${Date.now()}`;
  const report = {
  ...
  try {
    ...
    diagnosticStage("deployment-configured");
    await attachDiagnosticRpcTrace(provider);
    diagnosticStage("post-configuration-rpc-trace-attached");
    ...
    diagnosticStage("verify-live-wiring:start");
    const wiring = await verifyLiveWiring(ctx);
    diagnosticStage("verify-live-wiring:complete");
    ...
    const feeDistributor = new ethers.Contract(config.stakeDao.feeDistributor, FEE_DISTRIBUTOR_ABI, provider);
    const liveRewardToken = new ethers.Contract(wiring.rewardTokenAddress, ERC20_ABI, provider);
    diagnosticStage("token-metadata:start");
    report.tokens = {
    ...
    diagnosticStage("token-metadata:complete");

    diagnosticStage("reward-configuration:start");
    report.rewardConfiguration = await rewardConfigurationSnapshot(
      config,
      contracts,
      wiring.rewardSupportedByConverter
    );
    diagnosticStage("reward-configuration:complete");
    ...
    diagnosticStage("real-sdt-funding:start");
    report.sdtFunding = await fundErc20FromRealHolder({
      ...
      candidates: [
        ...
        config.stakeDao.feeDistributor,
        config.stakeDao.boostMarketplace
      ],
      lookbackBlocks: 10_000,
      maxWindows: 8
    });
    diagnosticStage("real-sdt-funding:complete");
    ...
    const directReward = await balanceDelta(
      ...
      () => sendTx(
        ...
        [actors.directVlSdtUser.address, actors.directVlSdtReceiver.address],
    ...
    reportPath = path.join(ROOT, "deployment-output-v18", `${tag}.json`);
    ...
    if (!reportPath) {
      reportPath = path.join(ROOT, "deployment-output-v18", `${tag}-failed.json`);
