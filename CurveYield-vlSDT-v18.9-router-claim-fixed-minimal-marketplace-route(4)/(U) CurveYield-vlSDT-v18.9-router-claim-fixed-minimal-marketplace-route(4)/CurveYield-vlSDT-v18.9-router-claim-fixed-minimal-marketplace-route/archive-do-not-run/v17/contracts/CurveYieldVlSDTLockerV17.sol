...
import {IVlSDTV17, IVlBoostV17, IFeeDistributorV17, IBoostMarketplaceV17} from "./interfaces/IStakeDaoV17.sol";
import {
    ICurveYieldVlSDTTokenV17,
    ICurveYieldVlSDTRevenueStakingV17
} from "./interfaces/ICurveYieldV17.sol";

contract CurveYieldVlSDTLockerV17 is Ownable2Step, ReentrancyGuard {
    ...
    IVlSDTV17 public immutable VLSDT;
    IVlBoostV17 public immutable VLBOOST;
    IFeeDistributorV17 public immutable FEE_DISTRIBUTOR;
    IBoostMarketplaceV17 public immutable BOOST_MARKETPLACE;
    ICurveYieldVlSDTTokenV17 public immutable CYVLSDT;
    ...
    constructor(
        ...
        if (
            initialOwner_ == address(0) || initialTreasuryReceiver_ == address(0)
                ...
                || feeDistributor_ == address(0) || boostMarketplace_ == address(0)
                || cyvlSdt_ == address(0)
        ) revert ZeroAddress();

        treasuryReceiver = initialTreasuryReceiver_;
        SDT = IERC20(sdt_);
        VLSDT = IVlSDTV17(vlSdt_);
        VLBOOST = IVlBoostV17(vlBoost_);
        FEE_DISTRIBUTOR = IFeeDistributorV17(feeDistributor_);
        BOOST_MARKETPLACE = IBoostMarketplaceV17(boostMarketplace_);
        CYVLSDT = ICurveYieldVlSDTTokenV17(cyvlSdt_);
    ...
    modifier onlyAdmin() {
        if (!systemConfigured) revert SystemNotConfigured();
        if (msg.sender != ICurveYieldVlSDTRevenueStakingV17(revenueStaking).admin()) revert OnlyAdmin();
        _;
    }
    ...
    function claimVlSDTRewards() external nonReentrant configured returns (uint256 claimed) {
        address rewardToken = FEE_DISTRIBUTOR.REWARD_TOKEN();
        uint256 beforeBalance = IERC20(rewardToken).balanceOf(address(this));
        FEE_DISTRIBUTOR.claim(address(this), address(this));
        claimed = IERC20(rewardToken).balanceOf(address(this)) - beforeBalance;
        if (claimed == 0) revert NoRewardClaimed();
        ...
        ICurveYieldVlSDTRevenueStakingV17(revenueStaking).notifyReward(
            rewardToken,
            claimed,
            baseRewardPerVlSDT
        );
    ...
    function admin() public view returns (address) {
        ...
        return ICurveYieldVlSDTRevenueStakingV17(revenueStaking).admin();
    }
    ...
    function acceptDaoMarketplaceOffer(
        ...
        IBoostMarketplaceV17.BuyOffer memory offer = BOOST_MARKETPLACE.getOffer(offerId);
    ...
    function _forwardRevenue(address paymentToken, uint256 amount) internal {
        ...
        ICurveYieldVlSDTRevenueStakingV17(revenueStaking).notifyReward(paymentToken, amount, 0);
