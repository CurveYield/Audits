...
async function deployLockerFixture() {
  ...
  const Gov = await ethers.getContractFactory("CurveYieldGovernanceTokenV17");
  const gov = await Gov.deploy(dao.address, "CurveYield Governance", "cyGOV");
  const BoostStaking = await ethers.getContractFactory("CurveYieldVlSDTBoostStakingV17");
...
describe("V17 requested design corrections", function () {
  ...
  it("keeps merchant prices inside DAO-configured per-token minimum and maximum bounds", async function () {
    ...
    const Merchant = await ethers.getContractFactory("CurveYieldVlSDTBoostMerchantV17");
